...
class Master extends DBConnection {
    ...
    public function __construct() {
        ...
        try {
            ...
            error_log("Erro ao inicializar Memcached: " . $e->getMessage());
        }
...
public function get_product($id) {
    ...
    try {
        ...
        if (!$product) {
            ...
            $query = "SELECT * FROM `product_list` WHERE id = '$id' AND delete_flag = 0";
            $result = $this->conn->query($query);
            
            if ($result && $result->num_rows > 0) {
                $product = $result->fetch_assoc();
                // Salva no cache por 1 hora
                $this->cache->set($cache_key, $product, 3600);
            }
        }
        ...
        // Em caso de erro com o cache, faz query direta
        $query = "SELECT * FROM `product_list` WHERE id = '$id' AND delete_flag = 0";
        $result = $this->conn->query($query);
...
public function get_order($id) {
    $cache_key = "order_" . $id;
    try {
        $order = $this->cache->get($cache_key);
        
        if (!$order) {
            $order = $this->conn->query("SELECT * FROM order_list WHERE id = '$id'")->fetch_assoc();
            if ($order) {
                $this->cache->set($cache_key, $order, 3600);
            }
        }
        ...
        return $this->conn->query("SELECT * FROM order_list WHERE id = '$id'")->fetch_assoc();
    }
}
private function getCotas($product_id) {
    // Inicializa o Memcached
    $memcached = new Memcached();
    $memcached->addServer('localhost', 11211);
    
    // Chave do cache
    $cache_key = "numeros_vendidos_produto_" . $product_id;
    
    // Tenta obter do cache primeiro
    $cotas_vendidas = $memcached->get($cache_key);
    ...
    if (!$cotas_vendidas) {
        ...
        $memcached->set($cache_key, $cotas_vendidas, 300);
    }
    
    return $cotas_vendidas;
}

private function clearCotasCache($product_id) {
    $memcached = new Memcached();
    $memcached->addServer('localhost', 11211);
    $cache_key = "numeros_vendidos_produto_" . $product_id;
...
//		return json_encode($resp);
	...
	{
	    error_log("Executando place_order em: " . __FILE__);
		...
		if (!$quantity) {
			...
			if ($resp['status'] == 'success') {
				$this->clearCotasCache($product_id);
			}	
			return json_encode($resp);
			exit();
		}
    ...
    $memcached = new Memcached();
    $memcached->addServer('localhost', 11211);
    $cache_key = "numeros_vendidos_manual_" . $product_id;
    $dados_cache = $memcached->get($cache_key);
